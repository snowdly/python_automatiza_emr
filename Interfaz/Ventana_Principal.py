...
def ventana():
    ...
    try:
        conn = mysql.connector.connect(user='admin_web_bqa', password='Password10_nae',
                                       host='85.25.194.35',
                                       database='web_bqa')
        curs = conn.cursor()
        ...
        curs.close()
        conn.close()
        results_for_modeloantena = [result[0] for result in results]
    except Exception as e:
        results_for_modeloantena = [""]
        messagebox.showerror("Error", "No se ha podido conectar a la Base de Datos para obtener datos de las Antenas")
        Bandera_Principal = "KO"
    ...
    btnCancelar = tk.Button(root, text="Cancelar", font=ftl, command = root.destroy)
