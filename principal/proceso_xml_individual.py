...
def principal(rootDir, rootResultados, nameFile, ficheros_respaldo):
    ...
    try:
        ...
        if len(fichero_cap) > 0:
            ...
            fichero_texto_cap = procesos_comunes.fichero_pdf_imagen_texto(fichero_cap[0], ficheros_respaldo, rootDir,
                                                                          'Conversion_Fichero_CAP')['Fichero_Texto']
    ...
    for fichero in procesos_comunes.lista_xml(rootDir):
        ...
        fichero_tecnico = \
        procesos_comunes.fichero_pdf_imagen_texto(fichero.replace('xml', 'pdf'), ficheros_respaldo, rootDir,
                                                  'Conversion_Fichero_Tecnico')['Fichero_Texto']
...
def principal_refactor(rootDir, rootResultados, nameFile, ficheros_respaldo):
    ...
    try:
        ...
        if fichero_tecnico_competente != '':
            fichero_tecnico_competente = \
            procesos_comunes.fichero_pdf_imagen_texto(fichero_tecnico_competente, ficheros_respaldo, rootDir,
                                                      'Conversion_Fichero_Tecnico_Competente')['Fichero_Texto']
        # r_datos_pdf = procesos_comunes.compara_tecnico_competente_pdf_texto(rootDir)
    except Exception as e2:
        fichero_tecnico_competente = ''
        print("Error en conversión de fichero TECNICO: ")
    ...
    for fichero in procesos_comunes.lista_xml(rootDir):
        ...
        # 3.1. VALIDACION SI EL XML ES CORRECTO
        lista = procesos_comunes.estructura_xml_completa(fichero)
        if len(lista) >= 1:
            ...
            for elemento in lista:
                try:
                    dv = reglas_validacion_new.reglas_validacion_individual(elemento['Etiqueta'], elemento['Regla'],
                                                                            '' if elemento['Valor'] is None else
                                                                            elemento['Valor'],
                                                                            fichero, ficheros_respaldo, rootDir,
                                                                            pdf_texto,
...
def genera_resumen_xml(rootDirResultados, nameFile):
    ...
    if len(array_df) >= 2:
        ...
        if unicos['Etiqueta'].count() > 1:
            writer = pd.ExcelWriter(
                os.path.join(rootDirResultados, nameFile + '_analisis_inidvidual_xml/',
                             nameFile + "_CONSOLIDADO_analisis.xlsx"))
