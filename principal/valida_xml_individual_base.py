...
def principal(rootDir, nameZip, carpeta):
    nameFile = ""
    ...
    with open(rootDir + nameZip + '.csv', 'w', newline='') as csvfile:
        ...
        for each in root.findall('.//Datos_Certificacion'):
            ...
            validacsv.writerow(['Datos_Certificacion', dato_Tipo_Certificacion.text,
                                regla.get("OK_KO"),regla.get("Validacion"), date.today()])
            ...
            validacsv.writerow(['Titular_Nombre_Razon_Social', dato_Expediente_Concesional,
                                regla.get("OK_KO"), regla.get("Validacion"), date.today()])
            bandera_mover = regla.get("Bandera")
        ...
        for each in root.findall('.//Datos_Visado'):
            ...
            validacsv.writerow(['Colegio_Profesional', dato_Colegio_Profesional.text,
                                ...
                                reglas_validacion.R_Colegio_Profesional(dato_Colegio_Profesional.text).get("Validacion"),
                                date.today()])
        ...
        for each in root.findall('.//Tecnico_Competente'):
            ...
            validacsv.writerow(
                ['NIF_NIE', dato_NIF_NIE.text, reglas_validacion.R_NIF_NIE(dato_NIF_NIE.text).get("OK_KO"),
                 reglas_validacion.R_NIF_NIE(dato_NIF_NIE.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(
                ['Nombre', dato_Nombre.text, reglas_validacion.debe_estar_lleno(dato_Nombre.text).get("OK_KO"),
                 reglas_validacion.debe_estar_lleno(dato_Nombre.text).get("Validacion"), date.today()])

            dato_Apellido1 = each.find('.//Apellido1')
            print('' if dato_Apellido1 is None else dato_Apellido1.text)
            validacsv.writerow(
                ['Apellido1', dato_Apellido1.text, reglas_validacion.debe_estar_lleno(dato_Apellido1.text).get("OK_KO"),
                 reglas_validacion.debe_estar_lleno(dato_Apellido1.text).get("Validacion"), date.today()])

            dato_Apellido2 = each.find('.//Apellido2')
            print('' if dato_Apellido2 is None else dato_Apellido2.text)
            validacsv.writerow(
                ['Apellido2', dato_Apellido2.text, reglas_validacion.debe_estar_lleno(dato_Apellido2.text).get("OK_KO"),
                 reglas_validacion.debe_estar_lleno(dato_Apellido2.text).get("Validacion"), date.today()])
        ...
        for each in root.findall('.//Datos_Emplazamiento'):
            ...
            validacsv.writerow(['Codigo_Emplazamiento', dato_Codigo_Emplazamiento.text,
                                ...
                                reglas_validacion.debe_estar_lleno(dato_Codigo_Emplazamiento.text).get("Validacion"),
                                date.today()])
            ...
            validacsv.writerow(['Cod_INE_Termino_Municipal', dato_Cod_INE_Termino_Municipal.text,
                                reglas_validacion.debe_estar_lleno(dato_Cod_INE_Termino_Municipal.text).get("OK_KO"),
            ...
            validacsv.writerow(['Latitud', dato_Latitud.text, reglas_validacion.debe_estar_lleno(dato_Latitud.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Latitud.text).get("Validacion"), date.today()])

            dato_Longitud = each.find('.//Longitud')
            print('' if dato_Longitud is None else dato_Longitud.text)
            validacsv.writerow(['Longitud', dato_Longitud.text, reglas_validacion.debe_estar_lleno(dato_Longitud.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Longitud.text).get("Validacion"), date.today()])

            dato_Datum = each.find('.//Datum')
            print('' if dato_Datum is None else dato_Datum.text)
            validacsv.writerow(['Datum', dato_Datum.text, reglas_validacion.debe_estar_lleno(dato_Datum.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Datum.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(
                ['Cota_Terreno_Sobre_Nivel_Mar', dato_Cota_Terreno_Sobre_Nivel_Mar.text, reglas_validacion.debe_estar_lleno(dato_Cota_Terreno_Sobre_Nivel_Mar.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Cota_Terreno_Sobre_Nivel_Mar.text).get("Validacion"), date.today()])

        for each in root.findall('.//Calle'):
            dato_Poblacion = each.find('.//Poblacion')
            print('' if dato_Poblacion is None else dato_Poblacion.text)
            validacsv.writerow(['Poblacion', dato_Poblacion.text, reglas_validacion.debe_estar_lleno(dato_Poblacion.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Poblacion.text).get("Validacion"), date.today()])

            dato_Tipo_Via = each.find('.//Tipo_Via')
            print('' if dato_Tipo_Via is None else dato_Tipo_Via.text)
            validacsv.writerow(['Tipo_Via', dato_Tipo_Via.text, reglas_validacion.debe_estar_lleno(dato_Tipo_Via.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Tipo_Via.text).get("Validacion"), date.today()])

            dato_Nombre_Via = each.find('.//Nombre_Via')
            print('' if dato_Nombre_Via is None else dato_Nombre_Via.text)
            validacsv.writerow(['Nombre_Via', dato_Nombre_Via.text, reglas_validacion.debe_estar_lleno(dato_Nombre_Via.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Nombre_Via.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(['Situacion', dato_Situacion.text, reglas_validacion.debe_estar_lleno(dato_Situacion.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Situacion.text).get("Validacion"), date.today()])
        ...
        for each in root.findall('.//Datos_Estacion'):
            ...
            validacsv.writerow(['Codigo_Estacion', dato_Codigo_Estacion.text, reglas_validacion.debe_estar_lleno(dato_Codigo_Estacion.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Codigo_Estacion.text).get("Validacion"), date.today()])

            dato_Tipo_Sistema = each.find('.//Tipo_Sistema')
            print('' if dato_Tipo_Sistema is None else dato_Tipo_Sistema.text)
            validacsv.writerow(['Tipo_Sistema', dato_Tipo_Sistema.text, reglas_validacion.R_Tipo_Sistema(dato_Tipo_Sistema.text).get("OK_KO"), reglas_validacion.R_Tipo_Sistema(dato_Tipo_Sistema.text).get("Validacion"), date.today()])

            dato_Tipo_Estacion = each.find('.//Tipo_Estacion')
            print('' if dato_Tipo_Estacion is None else dato_Tipo_Estacion.text)
            validacsv.writerow(['Tipo_Estacion', dato_Tipo_Estacion.text, reglas_validacion.R_Tipo_Estacion(dato_Tipo_Estacion.text).get("OK_KO"), reglas_validacion.R_Tipo_Estacion(dato_Tipo_Estacion.text).get("Validacion"), date.today()])

            dato_Entorno_Sensible = each.find('.//Entorno_Sensible')
            print('' if dato_Entorno_Sensible is None else dato_Entorno_Sensible.text)
            validacsv.writerow(['Entorno_Sensible', dato_Entorno_Sensible.text, reglas_validacion.debe_estar_lleno(dato_Entorno_Sensible.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Entorno_Sensible.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(['Num_Sectores_Interiores', dato_Num_Sectores_Interiores.text, reglas_validacion.debe_estar_lleno(dato_Num_Sectores_Interiores.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Num_Sectores_Interiores.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(['Num_Sectores_Exteriores', dato_Num_Sectores_Exteriores.text, reglas_validacion.debe_estar_lleno(dato_Num_Sectores_Exteriores.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Num_Sectores_Exteriores.text).get("Validacion"), date.today()])
        ...
        for each in root.findall('.//Sector'):
            ...
            validacsv.writerow(['Tipo_Potencia_Radiada', dato_Tipo_Potencia_Radiada.text, reglas_validacion.R_Tipo_Potencia_Radiada(dato_Tipo_Potencia_Radiada.text).get("OK_KO"), reglas_validacion.R_Tipo_Potencia_Radiada(dato_Tipo_Potencia_Radiada.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(['Potencia_Maxima_Total', dato_Potencia_Maxima_Total.text, reglas_validacion.debe_estar_lleno(dato_Potencia_Maxima_Total.text).get("OK_KO"), reglas_validacion.debe_estar_lleno(dato_Potencia_Maxima_Total.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(
                ['Unidad_Potencia_Maxima_Total', dato_Unidad_Potencia_Maxima_Total.text, reglas_validacion.R_Unidad_Potencia_Maxima_Total(dato_Unidad_Potencia_Maxima_Total.text).get("OK_KO"), reglas_validacion.R_Unidad_Potencia_Maxima_Total(dato_Unidad_Potencia_Maxima_Total.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(['Localizacion_Antena', dato_Localizacion_Antena.text, reglas_validacion.R_Localizacion_Antena(dato_Localizacion_Antena.text).get("OK_KO"), reglas_validacion.R_Localizacion_Antena(dato_Localizacion_Antena.text).get("Validacion"), date.today()])

            dato_Tipo_Antena = each.find('.//Tipo_Antena')
            print('' if dato_Tipo_Antena is None else dato_Tipo_Antena.text)
            validacsv.writerow(['Tipo_Antena', dato_Tipo_Antena.text, reglas_validacion.R_Tipo_Antena(dato_Tipo_Antena.text).get("OK_KO"), reglas_validacion.R_Tipo_Antena(dato_Tipo_Antena.text).get("Validacion"), date.today()])

            dato_Polarizacion = each.find('.//Polarizacion')
            print('' if dato_Polarizacion is None else dato_Polarizacion.text)
            validacsv.writerow(['Polarizacion', dato_Polarizacion.text, reglas_validacion.R_Polarizacion(dato_Polarizacion.text).get("OK_KO"), reglas_validacion.R_Polarizacion(dato_Polarizacion.text).get("Validacion"), date.today()])
            ...
            validacsv.writerow(
                ['Altura_Antena_Sobre_Terreno', dato_Altura_Antena_Sobre_Terreno.text, '', '', date.today()])
        ...
        for each in root.findall('.//Medida_Fase1'):
            ...
            validacsv.writerow(
                ['Valor_Medido_Promediado_Vm', dato_Valor_Medido_Promediado_Vm.text, '', '', date.today()])
    ...
    # MOVER FICHEROS
    #bandera_mover = 1
    print("Valor final de bandera mover: " + str(bandera_mover))
    if bandera_mover == 0:  # No existen errores
        shutil.move(rootDir + nameZip + ".csv", correctosDir + nameZip + ".csv")
        shutil.move(rootDir + nameZip + ".xlsx", correctosDir + nameZip + ".xlsx")
        shutil.move(rootDir + nameZip + ".zip", correctosDir + nameZip)
        ...
        shutil.move(rootDir + nameZip + ".zip", reparosDir + nameZip)
        ...
        shutil.move(rootDir + nameZip + ".zip", rechazosDir + nameZip)
