...
def infoantenas(Longitud,Latitud,Json):
	...
	#leemos json
	#with open('C:\Users\bsans\Downloads\infoantenasGeoJSON(23).do') as file:
	    #Json = json.load(file)
	#falta
	...
	for Cadena in Json:
		...
		if Cadena.find('@@<url-aplicacion') > 0:
			#que abra tantas paginas de detalle como url en json
			Estacion=Cadena
			print(Estacion)
			Url_Estacion= Estacion.split("Estacion")
			#limpiamos  comilla doble
			Url_Estacion = Url_Estacion[1].replace('"', '')
			#nueva ventana 
			driver =webdriver.Firefox(executable_path='./Ficheros_Respaldo/geckodriver.exe')
			driver.maximize_window()

			driver.get('https://geoportal.minetur.gob.es/VCTEL/detalleEstacion'+Url_Estacion)
